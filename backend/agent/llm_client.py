...
class LLMClient:
    ...
    async def generate(self, prompt: str, max_tokens: int = 2048) -> dict:
        """
        Generate response from LLM using Groq API.
        """
    ...
    async def generate_json(self, prompt: str) -> dict:
        ...
        """
        # Start LangFuse span for JSON generation
        if self.langfuse:
...
class CopilotClient:
    ...
    """
    def __init__(self, api_key: str | None = None, base_url: str = "https://api.githubcopilot.com", model: str = "grok-code-fast-1", langfuse_tracker=None):
    ...
    async def generate(self, prompt: str, system_prompt: str = "You are a helpful assistant.", max_tokens: int | None = None) -> dict:
        ...
        result = {"text": text, "tokens": tokens, "raw": resp, "time": elapsed}
        
        # Log to LangFuse
        if self.langfuse:
            self.langfuse.log_generation(
                name="copilot_generation",
                prompt=prompt,
                completion=text or "",
                model=self.model,
                tokens=tokens or 0,
    ...
    async def generate_json(self, prompt: str) -> dict:
        ...
        try:
            ...
            # End LangFuse span with error
            if self.langfuse:
                self.langfuse.end_span(metadata={"error": str(e)})
        ...
        # End LangFuse span with error
        if self.langfuse:
            self.langfuse.end_span(metadata={"error": "json_parse_failed", "json_parsed": False})
